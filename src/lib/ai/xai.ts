...
export async function callXAI(context: FullMarketContext): Promise<{
  success: boolean;
  ...
  if (!apiKey) {
    return { success: false, data: null, raw: '', error: 'XAI_API_KEY not set' };
  }
  ...
  try {
    const response = await undiciFetch(XAI_API, {
      method: 'POST',
      dispatcher,
      headers: {
